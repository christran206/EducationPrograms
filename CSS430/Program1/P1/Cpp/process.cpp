...
Specification
Creating a C++ program that mimics the command of 
	"ps -A | grep argv[1] | wc -l"
Command will get all processes, filter only process that match argument, 
	and then give a final count
...
Grand Child process executes grep argv[1] using read end of pipe from 
	Great Grand Child and outputs to write end of pipe to Child
Great Grand Child process executes ps -A and outputs to write end of 
	pipe to Grand Child
...
Step 4 create pipe and fork (keep track of pipes using vector)
step 5 repeat step 2 using (pipe loc-1) as end and (pipe loc-2) as start
