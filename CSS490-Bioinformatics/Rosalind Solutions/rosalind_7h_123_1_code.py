...
if __name__ == '__main__':
    ...
    suffArray = infile.readline()
    lcpArray = infile.readline()

    suffArray = map(int,suffArray.strip().split(','))
    lcpArray = map(int,lcpArray.strip().split(','))

    suffTree = SuffixArrayToTree(sequence,suffArray,lcpArray)
    edges = (suffTree.edge_word(e) for e in suffTree.edges.values())
